...
class MLP:
    ...
    def train(self, X, y, learning_rate, epochs, validation=None, batch_size=None, momentum=0.95):
        ...
        for epoch in range(epochs):
            ...
            for i in range(n_batches):
                ...
                self.v2 = momentum*self.v2 - learning_rate*grad_L_W2
                self.v1 = momentum*self.v1 - learning_rate*grad_L_W1

                self.W2 += momentum*self.v2 - learning_rate*grad_L_W2
                self.b2 -= learning_rate*grad_L_b2
                self.W1 += momentum*self.v1 - learning_rate*grad_L_W1
                self.b1 -= learning_rate*grad_L_b1
                
                # Updating the progress
                print_progress(i+1, n_batches, label="Batches", info=f"Epoch: {epoch}")
...
if __name__ =="__main__":
    ...
    # Nesterov momentum -> miu = 0.95                 
    model = MLP(X_train.shape[1], 100, y_train_encoded.shape[1])
    momentum_train_loss, _, momentum_train_error, _ = model.train(X_train, y_train_encoded, 
                                                               learning_rate=0.00001, 
                                                               epochs=epochs, 
                                                               validation=(X_test, y_test_encoded),
                                                               batch_size=4096)
    ...
    plt.plot(momentum_train_loss, label="with momentum")
    plt.grid()
    plt.legend()
    ...
    plt.plot(momentum_train_error, label="with momentum")
    plt.grid()
    plt.legend()
